...
async function main() {
  ...
  const organization = await authJsonRequest<{
    id: string;
    name: string;
    slug: string;
  }>(
  ...
  const otherOrganization = await authJsonRequest<{
    id: string;
    name: string;
    slug: string;
  }>(
  ...
  console.info(`Created organization ${organization.data.id}`);
  console.info(`Created second organization ${otherOrganization.data.id}`);

  const createdKey = await appJsonRequest<{
    key: { id: string; key: string; organizationId: string };
  }>(
    "/api/keys",
    {
  ...
  const createWorkspace = await appJsonRequest<{
  ...
  if (createWorkspace.data.workspace.organizationId !== organization.data.id) {
    throw new Error("API key created a workspace in the wrong organization.");
  }

  console.info(`Created workspace ${createWorkspace.data.workspace.id}`);

  await appRequest(
    `/workspaces?organizationId=${encodeURIComponent(organization.data.id)}`,
    {
      bearerToken: createdKey.data.key.key,
      expectedStatus: 200,
    },
  );
  console.info("API key can read its pinned organization.");

  await appRequest(
    `/workspaces?organizationId=${encodeURIComponent(otherOrganization.data.id)}`,
    {
      bearerToken: createdKey.data.key.key,
      expectedStatus: 403,
    },
  );
  console.info("API key is rejected for a different organization.");

  await appRequest(
    `/organizations/${encodeURIComponent(otherOrganization.data.id)}/access-targets`,
    {
      bearerToken: createdKey.data.key.key,
      expectedStatus: 403,
    },
  );
  console.info("API key is rejected for mismatched organization params.");
