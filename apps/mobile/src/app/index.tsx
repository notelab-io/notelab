...
export default function HomeScreen() {
  ...
  const { data: rawWorkspaces = [], isPending: isWorkspacesPending } =
    useWorkspaces();
  ...
  const workspaceId =
    ...
    workspaces[0]?.id ??
    null;
  const { data: navigation, isPending: isPagesPending } =
  ...
  const hasItems =
    sections.privateItems.length > 0 || sections.teamspaceItems.length > 0;
        ...
        {isLoading ? (
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>
              Create a page on web or from another device, then it will appear
              here with the same emoji and nesting structure.
            </Text>
          </View>
        )}
      </View>
...
function buildPageSections(
  ...
  for (const database of databases) {
    const page = database.pageId ? pagesById.get(database.pageId) : undefined;
  ...
  const buildPageNode = (
    ...
    return {
      ...baseNode,
      id: nodeId,
      children: (placementsByPageParent.get(pageId) ?? []).flatMap(
        (placement) => {
  ...
  const buildDatabaseNode = (
    ...
    return {
      ...baseNode,
      id: nodeId,
      children: (placementsByDatabaseParent.get(databaseId) ?? []).flatMap(
        (placement) => {
          if (placement.itemKind !== "page") {
            return [];
          }
