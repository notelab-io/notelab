    ...
    <div className="space-y-4">
      <div className="rounded-lg border bg-card p-4 text-card-foreground shadow-xs">
            ...
            <div className="min-w-0 space-y-2">
              <div className="space-y-1">
                <div className="flex flex-wrap items-center gap-2">
                  <h4 className="font-medium">Linear workspace</h4>
                  <ConnectionBadge connected={status?.workspace.connected} />
                </div>
                <p className="max-w-xl text-sm text-muted-foreground">
                  Admin-managed Linear organization connection. Members can
                  connect their own Linear account after this workspace is
                  connected.
                </p>
              </div>
              <div className="grid gap-2 text-sm md:grid-cols-2">
                <IntegrationDetail
                  label="Workspace"
                  value={
                    status?.workspace.organizationName || "Not connected"
                  }
                />
                <IntegrationDetail
                  label="Workspace ID"
        ...
        <div className="mt-4 flex items-center justify-between gap-4 rounded-md border bg-background px-3 py-2">
          <div className="space-y-0.5">
            ...
            <div className="text-xs text-muted-foreground">
              Members must connect a Linear account using their Notelab
              organization email.
            </div>
          </div>
          <Switch
        ...
        {!isWorkspaceConnected && !canManageWorkspace ? (
          <Alert className="mt-4">
            <AlertDescription>
              Linear workspace is not connected. Ask an admin to connect it
              before linking your Linear account.
            </AlertDescription>
          </Alert>
        ) : null}
        {status?.configured === false ? (
      ...
      <div className="rounded-lg border bg-card p-4 text-card-foreground shadow-xs">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="flex min-w-0 gap-3">
            ...
            <div className="min-w-0 space-y-2">
              <div className="space-y-1">
                <div className="flex flex-wrap items-center gap-2">
                  <h4 className="font-medium">My Linear account</h4>
                  <ConnectionBadge connected={status?.personal.connected} />
                </div>
                <p className="max-w-xl text-sm text-muted-foreground">
                  Connect your own Linear identity so AI can read the Linear
                  items your account can access.
                </p>
              </div>
              <div className="grid gap-2 text-sm md:grid-cols-2">
                <IntegrationDetail
        ...
        {!isWorkspaceConnected ? (
          <Alert className="mt-4">
            <AlertDescription>
              Linear workspace is not connected yet.
            </AlertDescription>
          </Alert>
        ) : null}
      </div>
