...
export const workspaceIntegration = pgTable(
  ...
  (table) => [
    uniqueIndex("workspace_integration_account_idx").on(
      table.workspaceId,
      table.integrationKey,
      table.providerAccountId,
    ),
    index("workspace_integration_workspace_idx").on(table.workspaceId),
    index("workspace_integration_key_idx").on(
      table.workspaceId,
      table.integrationKey,
    ),
    index("workspace_integration_status_idx").on(table.status),
  ],
);
...
export const userIntegration = pgTable(
  ...
  (table) => [
    ...
    index("user_integration_workspace_idx").on(table.workspaceId),
    index("user_integration_user_idx").on(table.userId),
    index("user_integration_key_idx").on(
      table.workspaceId,
      table.integrationKey,
    ),
    index("user_integration_provider_workspace_idx").on(
      table.workspaceId,
      table.integrationKey,
...
export const commentThread = pgTable(
  ...
  (table) => [
    index("comment_thread_page_deleted_idx").on(
      table.pageId,
      table.deletedAt,
    ),
    index("comment_thread_workspace_page_idx").on(
      table.workspaceId,
      table.pageId,
    ),
    index("comment_thread_page_state_activity_idx").on(
      table.pageId,
      table.resolvedAt,
...
export const commentMessage = pgTable(
  "comment_message",
  {
    id: text("id").primaryKey(),
  ...
  (table) => [
    index("comment_message_thread_created_idx").on(table.threadId, table.createdAt),
  ],
);
...
export const commentReaction = pgTable(
  "comment_reaction",
  {
    ...
    messageId: text("message_id")
      .notNull()
      .references(() => commentMessage.id, { onDelete: "cascade" }),
    userId: text("user_id")
      .notNull()
      .references(() => user.id, { onDelete: "cascade" }),
  ...
  (table) => [
    index("comment_reaction_message_idx").on(table.messageId),
    uniqueIndex("comment_reaction_message_user_emoji_unique").on(
      table.messageId,
      table.userId,
      table.emoji,
    ),
