import { LinearReadonlyClient } from "@notelab/connectors/linear";
...
export function buildLinearTools(accessToken: string): ToolSet {
  ...
  return {
    getLinearProfile: tool({
      description:
        "Read the connected Linear page and user profile for the current user's Linear account.",
    ...
    listLinearProjects: tool({
      description:
        "List Linear projects visible to the connected workspace integration.",
      inputSchema: z.object({
        cursor: z.string().trim().optional(),
        limit: z.number().int().min(1).max(50).default(20),
      }),
    ...
    listLinearIssues: tool({
      description:
        "List recent Linear issues visible to the connected workspace integration. Prefer searchLinearIssues when the user gives a topic or keyword.",
      inputSchema: z.object({
        cursor: z.string().trim().optional(),
        limit: z.number().int().min(1).max(20).default(10),
      }),
