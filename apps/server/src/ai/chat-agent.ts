...
export class ChatAgent extends AIChatAgent<ChatAgentEnv> {
  ...
  async persistMessages(
    ...
    await this.withDb(async () => {
      await syncAiChatThreadMessages(this.activeThreadId!, this.messages, {
  ...
  protected async onChatResponse(result: ChatResponseResult) {
    if (!this.activeThreadId || result.status !== "completed") {
      return;
    }

    await this.withDb(async () => {
      await touchAiChatThreadActivity(this.activeThreadId!);
      await maybeAutoTitleAiChatThread(this.activeThreadId!, this.messages);
    });
  }
  ...
  async onChatMessage(
    ...
    const requestBody = coerceRequestBody(options?.body);
    ...
    const auth = await this.withDb(async () => {
    ...
    this.activeThreadId = auth.threadId;

    const hydration = await this.withDb(async () =>
      ensureThreadHydrated({
        loadMessages: () => loadAiChatThreadMessages(auth.threadId),
        localMessages: this.messages,
      }),
    );

    if (hydration.shouldHydrate) {
      await super.persistMessages(hydration.messages, [], { _deleteStaleRows: true });
    }
    ...
    const hasPageEditAccess =
      hasPageContext && requestBody.canEditPages;

    if (hasPageContext) {
      console.warn(
        `Chat agent page edit: canEdit=${requestBody.canEditPages} allowedIds=${allowedPageIds.join(",") || "(none)"} primaryId=${requestBody.primaryPageId ?? "(none)"}`,
      );
    }
    ...
    const model = resolveOpenAiChatModel(this.env.OPENAI_API_KEY, requestBody.model);
    ...
    const result = streamText({
      ...
      onError: ({ error }) => {
        console.warn(
          `Chat agent ${authUserId}: ${toProviderErrorMessage(error)}`,
        );
      },
      onFinish,
    });
  ...
  private withDb<T>(fn: () => Promise<T>) {
    return runWithDbClient(
      createDbClient(this.env as ChatAgentEnv & { [key: string]: unknown }),
      fn,
    );
  }
}
...
function coerceRequestBody(body: unknown): {
  allowedPageIds: string[];
...
function ensureThreadHydrated(input: {
  loadMessages: () => Promise<UIMessage[]>;
  localMessages: readonly UIMessage[];
}) {
  return input.loadMessages().then((postgresMessages) => {
    ...
    const localIds = new Set(input.localMessages.map((message) => message.id));
    const shouldHydrate =
      postgresMessages.length !== input.localMessages.length ||
      postgresMessages.some((message) => !localIds.has(message.id));
